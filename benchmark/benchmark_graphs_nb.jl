...
begin
	...
	function getbenches(res)
		_res = res[2][1]
		res_df = DataFrame(Category=String[],T=[],Bench=String[], Time=Float64[])
		_getbenches!(res_df, [], _res)
		return res_df
	end
	function _getbenches!(df, vals, res)
		# @info res
		if res[1] == "BenchmarkGroup"
			# @show res[2]
			for (key,value) in res[2]["data"]
				_getbenches!(df, push!(copy(vals),key), value)
			end
		elseif res[1] == "TrialEstimate"
			# @show vals
			# @show res[2]["time"]
			push!(df, (Category=vals[1], T=vals[2],Bench=vals[3],Time=res[2]["time"]))
		end
	end
...
md"
Test: $(@bind cat Select(unique(benchres_df.Category);))
"
...
manifest_format = "2.0"
project_hash = "baee9e8f260630c1154d35b552a51b54a1819d5e"
