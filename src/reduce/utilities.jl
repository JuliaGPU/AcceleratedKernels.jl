...
function _mapreduce_nd_apply_init!(
    ...
    foreachindex(dst, backend; max_tasks, min_elems, block_size) do i
        @inbounds dst[i] = op(init, f(@inbounds src[i]))
    end
end
...
@inline function reduce_group!(@context, op, sdata, N, ithread)
    if N >= 512u16
        if ithread < 256u16
            @inbounds sdata[ithread + 0x1] = op(@inbounds(sdata[ithread + 0x1]), @inbounds(sdata[ithread + 256u16 + 0x1]))
        end
        @synchronize()
    end
    if N >= 256u16
        if ithread < 128u16
            @inbounds sdata[ithread + 0x1] = op(@inbounds(sdata[ithread + 0x1]), @inbounds(sdata[ithread + 128u16 + 0x1]))
        end
        @synchronize()
    end
    if N >= 128u16
        if ithread < 64u16
            @inbounds sdata[ithread + 0x1] = op(@inbounds(sdata[ithread + 0x1]), @inbounds(sdata[ithread + 64u16 + 0x1]))
        end
        @synchronize()
    end
    if N >= 64u16
        if ithread < 32u16
            @inbounds sdata[ithread + 0x1] = op(@inbounds(sdata[ithread + 0x1]), @inbounds(sdata[ithread + 32u16 + 0x1]))
        end
        @synchronize()
    end
    if N >= 32u16
        if ithread < 16u16
            @inbounds sdata[ithread + 0x1] = op(@inbounds(sdata[ithread + 0x1]), @inbounds(sdata[ithread + 16u16 + 0x1]))
        end
        @synchronize()
    end
    if N >= 16u16
        if ithread < 8u16
            @inbounds sdata[ithread + 0x1] = op(@inbounds(sdata[ithread + 0x1]), @inbounds(sdata[ithread + 8u16 + 0x1]))
        end
        @synchronize()
    end
    if N >= 8u16
        if ithread < 4u16
            @inbounds sdata[ithread + 0x1] = op(@inbounds(sdata[ithread + 0x1]), @inbounds(sdata[ithread + 4u16 + 0x1]))
        end
        @synchronize()
    end
    if N >= 4u16
        if ithread < 2u16
            @inbounds sdata[ithread + 0x1] = op(@inbounds(sdata[ithread + 0x1]), @inbounds(sdata[ithread + 2u16 + 0x1]))
        end
        @synchronize()
    end
    if N >= 2u16
        if ithread < 1u16
            @inbounds sdata[ithread + 0x1] = op(@inbounds(sdata[ithread + 0x1]), @inbounds(sdata[ithread + 1u16 + 0x1]))
        end
        @synchronize()
    end
end
